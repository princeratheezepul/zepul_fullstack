...
function AccountManagerRegister() {
 ...
 const handleSubmit = async (e) => {
  ...
  try {
    ...
    console.log(`${import.meta.env.VITE_BACKEND_URL}/api/accountmanager/register`)
    const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/accountmanager/register`, {
      method: "POST",
      headers: {
      ...
      <div className="w-full max-w-xs sm:max-w-sm md:max-w-md lg:max-w-lg bg-blue-600 rounded-xl shadow-lg p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <p className="mt-4 text-white text-center">
            ...
            <Link to="/accountmanager/login" className=" font-semibold hover:underline">
              Login
            </Link>
          </p>
