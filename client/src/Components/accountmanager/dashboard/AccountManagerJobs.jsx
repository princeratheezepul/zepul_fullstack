...
import AccountManagerJobCard from './AccountManagerJobCard';
import AccountManagerJobDetails from './AccountManagerJobDetails';
import AccountManagerCandidateList from './AccountManagerCandidateList';
import AccountManagerCreateJob from './AccountManagerCreateJob';
import AccountManagerJobSidebar from './AccountManagerJobSidebar';
...
const AccountManagerJobs = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      delete window.refreshAccountManagerJobs;
    };
  }, [currentPage, activeFilter]);
  ...
  const fetchJobs = async () => {
    try {
      ...
      if (!accountManagerId) {
        console.error('No accountManagerId found in user info');
        return;
      }

      // Use the account manager jobs endpoint
      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/accountmanager/getjob`, {
        method: 'GET',
        headers: {
      ...
      if (response.ok) {
        ...
        // Filter jobs to show only those created by AccountManager or all jobs
        // Since AccountManager should see all jobs, we don't need to filter by creator
  ...
  const handleJobClick = (job) => {
    navigate(`/accountmanager/jobs/${job._id}`);
  };
  ...
  if (showCreateJob) {
    return (
      <div className="w-full min-h-screen bg-[#F7F8FA]">
        <AccountManagerCreateJob onBack={() => setShowCreateJob(false)} />
      </div>
    );
  }
  ...
  return (
    <div className="bg-gray-50 w-full px-2 sm:px-4 md:px-8 lg:px-16 py-0 min-h-screen relative">
        ...
        <>
          <div>
            {jobs.length === 0 ? (
              ...
              jobs.map((job) => (
                <AccountManagerJobCard key={job._id || job.id} job={job} onClick={handleJobClick} />
              ))
            )}
          </div>
      ...
      <AccountManagerJobSidebar open={!!selectedJob} onClose={handleBack}>
        {selectedJob && !showCandidateList && (
          <AccountManagerJobDetails job={selectedJob} onBack={handleBack} onShowCandidates={handleShowCandidates} onJobUpdated={handleJobUpdated} />
        )}
        {selectedJob && showCandidateList && (
          <AccountManagerCandidateList job={selectedJob} onBack={handleBackToJobDetails} />
        )}
      </AccountManagerJobSidebar>
    </div>
  );
};
